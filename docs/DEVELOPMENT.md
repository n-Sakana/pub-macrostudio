...
- **見た目の語彙は `assets/js/components.js` と `assets/css/components.css`
  ...
  解決すること。4 つ目を作らず、強調は太さと色で行う。総合判定の語句
  （`改修が必要` など）だけが `--type-display-size` を使い、これは 1 か所しか
  無い。`tests\test-design-tokens.js` が段数と 11 種の登録を検査する。
...
- **総合判定はブック全体に 1 つ**（SPEC §2.2.1.1）。`worstGrade()` が唯一の
  判定で、指摘 0 件は A。画面は `ui.verdict` を 1 つだけ描き、判定は等級の
  文字ではなく語句（`改修が必要` など）で言う。文字は `data-grade` とトーンに
  だけ残す。到達していない等級の色を出さない。等級ごとの件数カードを並べる形、
  および裸の 1 文字を主役に出す形へ戻さないこと（前者は 0 件の D が最も目立ち
  「このマクロは D」と読め、後者は凡例を読むまで何も言っていなかった）。
  区分カードにも行にも等級の文字チップを置かず、`GRADE_SECTIONS` の見出しで
  意味を言う。
- **変更範囲は `presets/03_変更範囲/` が持つ**。ファイルの `## 構造変更` が
  `禁止` か `許可` を名乗り、フォルダの先頭のファイルが既定になる。
  画面はこれを**操作カードではなく一般的な track+thumb スイッチ**として出す
  （`ui.toggleSwitch`、`role="switch"`。OFF=既定の禁止、ON=許可）。1 つの
  二択に控えを 2 つ置かないこと（既定のカード＋「詳細オプション」の
  折りたたみ、という形へ戻さない）。いまの値はスイッチ上の「いまの設定」に
  名前と説明で書き出し、状態は `許可する` / `許可しない` の語でも言う。
  モジュール単位出力の ON/OFF も同じ部品を使う（種類の違う切替を checkbox と
  スイッチで混在させない）。
...
node tests\test-intake-retry.js
node tests\test-code-max.js
...
- `tests\test-design-tokens.js` … デザインシステムの契約。UI の字は 3 段へ、
  ...
  track+thumb スイッチが `role="switch"` の実ボタンで、状態をその設定の語彙の
  語（`許可する` 等）でも言うこと。総合判定が語句であって裸の文字でないこと。
  警告が色以外の手掛かりを持つこと。撤去した 1 回限りの見た目
  （`.attachment-hint` 等）が戻っていないこと。
- `tests\test-intake-retry.js` … 2026-08 の実失敗（取り込み失敗が続くと 2 回で
  言い直し文を打ち切り、読者が返答を手で整形するしかなくなった）の回帰。
  異なる理由の連続 4 失敗すべてで、その回の失敗から作った言い直し文が
  クリップボードへ入ること、画面の［言い直す依頼文をコピー］が最新の文を
  何度でも書き直すこと、入力と依頼 ID が失敗で失われないこと、実際の失敗形
  （複数手続きを 1 件へ詰めた PROC）への拒否が「手続きごとに分ける」を言い
  「- にする」を勧めないこと、分割し直した返答が受理され画面で 1 つの問題へ
  束ね直されることを固定する。
- `tests\test-code-max.js` … 確認画面のアプリ内最大化。maximize / restore が
  アイコンだけの実ボタンで名前と状態（`aria-label` / `aria-pressed`）を語で
  持つこと、機構が `<body>` の class だけで再描画を経ないこと（スクロール・
  選択・手動修正の下書きが往復で失われない根拠）、手動修正ツールバーにも同じ
  操作があることを固定する。実窓での隠れる範囲・Esc・スクロール保持は
  `test-flow-webview.ps1` が見る。
