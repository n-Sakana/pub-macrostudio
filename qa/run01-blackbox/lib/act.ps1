...
$script:ST = Get-Content "$RUN\logs\app-state.json" -Raw | ConvertFrom-Json
...
function Shot {
  ...
  # Re-read the rect first: capturing with a stale one photographs whatever
  # now occupies those screen coordinates, which is how R2_022 ended up
  # showing a different window entirely.
...
function Tap {
  ...
  # The window can shrink between one step and the next. CurrentScale then
  # flips from 1.44 to 1.0 and a coordinate read off an older capture lands
  # OUTSIDE the window - still on screen, still with MacroStudio foreground,
  # so neither check above notices, and the click goes to whatever is behind.
  # That happened once: it clicked another window and the capture that
  # followed photographed that window instead. Refuse instead.
